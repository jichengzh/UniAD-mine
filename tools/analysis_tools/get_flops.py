...
def get_model_complexity_info(model,
    ...
    if input_constructor:
        ...
        # # data['img']是一个列表，列表里面存储着DataContainer对象。
        # # data['img'][0].data表示的是DataContainer对象的data属性，通常也是一个列表。这里是取了列表的第一个元素
        ...
        # ego_fut_trajs = data['ego_fut_trajs'][0].data[0].to(batch.device)
        # ego_fut_cmd = data['ego_fut_cmd'][0].data[0].to(batch.device)
        ...
        #                 fut_valid_flag=fut_valid_flag, ego_his_trajs=ego_his_trajs, 
        #                 ego_lcf_feat=ego_lcf_feat, ego_fut_trajs=ego_fut_trajs, 
        #                 ego_fut_cmd=ego_fut_cmd, gt_bboxes_3d=gt_bboxes_3d, 
...
def parse_args():
    ...
    # 点云模式下表示点数和特征维度，图像模式表示宽高，之后会转化为(3, H, W)
    parser.add_argument(
        '--shape',
        type=int,
        nargs='+',
    ...
    # 这里的modality表示输入数据的模态，point表示点云，image表示图像，multi表示多模态
    parser.add_argument(
        '--modality',
        type=str,
        default='image',
...
def main():
    ...
    # 输入模态是图像时，形状参数可以是一个整数或两个整数，表示长宽。
...
# 判断配置文件中是否有插件plugin，如果有则加载插件
    if hasattr(cfg, 'plugin'):
        if cfg.plugin:
            import importlib
            # 如果插件目录plugin_dir存在，则从该目录加载插件
    ...
    # isinstance的作用是判断cfg.data.test是否是一个list类型
    elif isinstance(cfg.data.test, list):
        for ds_cfg in cfg.data.test:
            ds_cfg.test_mode = True
        # 如果测试数据集是一个列表，则取第一个数据集的samples_per_gpu
        samples_per_gpu = max(
