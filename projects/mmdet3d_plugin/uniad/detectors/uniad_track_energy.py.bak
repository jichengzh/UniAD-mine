...
class UniADTrack(MVXTwoStageDetector):
    ...
    # def measure_energy_consumption_track(self, func, *args, **kwargs):
    #     """测量某个模块的能耗"""
    ...
    #     energy_consumed = (end_energy - start_energy)  # 转换为焦耳
    #         # 根据 func 的类型或名称进行标注
    #         # 根据 func 的类型或名称进行标注
    #     # module_name = str(func)
    #     print(f"    2.[Energy Consumption] Module: Energy consumed: {energy_consumed:.3f}")
    #     return result
    ...
    def extract_img_feat(self, img, len_queue=None):
        ...
        if self.use_grid_mask:
            # print("Measuring energy consumption for GridMask...")True
            img = self.energy_meter.measure_energy_consumption_track(self.grid_mask, False, 'GridMask', img)
        # print("Measuring energy consumption for img_backbone...")
        img_feats = self.energy_meter.measure_energy_consumption_track(self.img_backbone, False, 'img_backbone', img)
        if isinstance(img_feats, dict):
            img_feats = list(img_feats.values())
        if self.with_img_neck:
            # print("Measuring energy consumption for img_neck...")
            img_feats = self.energy_meter.measure_energy_consumption_track(self.img_neck, False, 'img_neck', img_feats)
    ...
    def get_history_bev(self, imgs_queue, img_metas_list):
        ...
        with torch.no_grad():
            ...
            for i in range(len_queue):
                ...
                prev_bev, _ = self.energy_meter.measure_energy_consumption_track(
                    self.pts_bbox_head.get_bev_features, False, 'pts_bbox_head.get_bev_features', 
    ...
    def get_bevs(self, imgs, img_metas, prev_img=None, prev_img_metas=None, prev_bev=None):
        if prev_img is not None and prev_img_metas is not None:
            ...
            # print("Measuring energy consumption for get_history_bev...")
            prev_bev = self.energy_meter.measure_energy_consumption_track(self.get_history_bev, False, 'get_history_bev', prev_img, prev_img_metas)

        # print("Measuring energy consumption for extract_img_feat...")
        img_feats = self.energy_meter.measure_energy_consumption_track(self.extract_img_feat, False, 'extract_img_feat', img=imgs)
        if self.freeze_bev_encoder:
            with torch.no_grad():
                # print("Measuring energy consumption for pts_bbox_head.get_bev_features (frozen)...")
                bev_embed, bev_pos = self.energy_meter.measure_energy_consumption_track(
                    self.pts_bbox_head.get_bev_features, False, 'get_bev_features (frozen)', 
                    mlvl_feats=img_feats, img_metas=img_metas, prev_bev=prev_bev
                )
        else:
            # print("Measuring energy consumption for pts_bbox_head.get_bev_features...")
            bev_embed, bev_pos = self.energy_meter.measure_energy_consumption_track(
                self.pts_bbox_head.get_bev_features, False, 'get_bev_features', 
                mlvl_feats=img_feats, img_metas=img_metas, prev_bev=prev_bev
            )
    ...
    def _forward_single_frame_train(
        ...
        # print("Measuring energy consumption for get_bevs...")
        bev_embed, bev_pos = self.energy_meter.measure_energy_consumption_track(
            self.get_bevs, False, 'get_bevs', img, img_metas, prev_img=prev_img, prev_img_metas=prev_img_metas
        )
        # print("Measuring energy consumption for pts_bbox_head.get_detections...")
        det_output = self.energy_meter.measure_energy_consumption_track(
            self.pts_bbox_head.get_detections, False, 'pts_bbox_head.get_detections', 
            bev_embed, object_query_embeds=track_instances.query, ref_points=track_instances.ref_pts, img_metas=img_metas
        )
        ...
        if l2g_r2 is not None:
            ...
            # print("Measuring energy consumption for velo_update...")
            ref_pts = self.energy_meter.measure_energy_consumption_track(
                self.velo_update,
                False, 
                'velo_update',
                last_ref_pts[0],
        ...
        for i in range(nb_dec):
            ...
            # print("Measuring energy consumption for criterion.match_for_single_frame...")
            track_instances, matched_indices = self.energy_meter.measure_energy_consumption_track(
                self.criterion.match_for_single_frame, False, 'match_for_single_frame', out, i, if_step=(i == (nb_dec - 1))
            )
    ...
    def select_sdc_track_query(self, sdc_instance, img_metas):
        out = dict()
        result_dict = self._track_instances2results(sdc_instance, img_metas, with_mask=True)
    ...
    def _forward_single_frame_inference(
        ...
        if l2g_r2 is not None and len(active_inst) > 0 and l2g_r1 is not None:
            ...
            # print("Measuring energy consumption for velo_update...")
            ref_pts = self.energy_meter.measure_energy_consumption_track(
                self.velo_update,False,'velo_update', ref_pts, velo, l2g_r1, l2g_t1, l2g_r2, l2g_t2, time_delta=time_delta
            )
        # print("Measuring energy consumption for get_bevs...")
        # bev_embed, bev_pos = self.get_bevs(img, img_metas, prev_bev=prev_bev)
        bev_embed, bev_pos = self.energy_meter.measure_energy_consumption_track(
            self.get_bevs,False,'get_bevs', img, img_metas, prev_bev=prev_bev
            )
        # print("Measuring energy consumption for pts_bbox_head.get_detections...")
        det_output = self.energy_meter.measure_energy_consumption_track(
            self.pts_bbox_head.get_detections,
            False, 
        ...
        # print("Measuring energy consumption for select_active_track_query...")
        out.update(self.energy_meter.measure_energy_consumption_track(
        ...
        if self.memory_bank is not None:
            # print("Measuring energy consumption for memory_bank...")
            track_instances = self.energy_meter.measure_energy_consumption_track(
                self.memory_bank, 
                False,
                'memory_bank',
                track_instances
        ...
        tmp = {}
        # print("Measuring energy consumption for _generate_empty_tracks...")
        tmp["init_track_instances"] = self.energy_meter.measure_energy_consumption_track(
        ...
        # tmp["init_track_instances"] = self._generate_empty_tracks()
        tmp["track_instances"] = track_instances
        # print("Measuring energy consumption for query_interact...")
        out_track_instances = self.energy_meter.measure_energy_consumption_track(
            self.query_interact,
            False,
            'query_interact',
            tmp
    ...
    def simple_test_track(
        ...
        print("simple_test_track is called")
        ...
        # print("Measuring energy consumption for _forward_single_frame_inference...")
        frame_res = self.energy_meter.measure_energy_consumption_track(
            self._forward_single_frame_inference,
            False,
            '_forward_single_frame_inference',
            img,
            img_metas,
            track_instances,
            prev_bev=prev_bev,
