# 修改策略：
# 增加fpn输出特征维度。增加了一个浅层特征方便检测小目标。
# 增加了resnet的感知深度。
# bevformer检测头方面没有改动
# 增加了motion head的query数量300到400
# 增加了motion head的transformer层数3到6
...
model = dict(
    ...
    img_backbone=dict(
        ...
        depth=152,  #增加感知深度
        num_stages=4,
        out_indices=(0, 1, 2, 3), # output all stages底层细节更加丰富
        frozen_stages=4,
    ...
    img_neck=dict(
        type="FPN",
        in_channels=[256, 512, 1024, 2048], # 怎加了底层细节
        # in_channels=[512, 1024, 2048],  --- IGN
        out_channels=_dim_,
        start_level=0,
    ...
    motion_head=dict(
        ...
        bev_w=bev_w_,
        # num_query=300,
        num_query=300,  # same as pts_bbox_head
        num_classes=10,
        ...
        transformerlayers=dict(
            ...
            embed_dims=_dim_,
            # num_layers=3,
            num_layers=6,  # same as pts_bbox_head
            transformerlayers=dict(
